...
import static ca.bc.gov.educ.grad.dto.reports.data.BusinessEntity.nullSafe;
import static ca.bc.gov.educ.grad.dto.reports.data.impl.DistrictOrganisation.LOGO_CODE_BC;
...
public class BusinessEntityAdapter {
    ...
    public static Certificate adapt(
            final ca.bc.gov.educ.grad.model.cert.Certificate certificate) {
    ...
    public static Student adapt(
            final ca.bc.gov.educ.grad.model.student.Student student) {
        validate(student, "student");

        final PostalAddress address = adapt(student.getCurrentMailingAddress());

        ca.bc.gov.educ.grad.dto.reports.data.impl.Student std = new Student.Builder()
    ...
    public static School adapt(
            final ca.bc.gov.educ.grad.model.school.School school,
    ...
    private static PostalAddress adapt(
            final ca.bc.gov.educ.grad.model.common.party.address.PostalAddress address) {
    ...
    public static void adapt(
            final ca.bc.gov.educ.grad.model.transcript.Transcript transcript,
        ...
        for (final ca.bc.gov.educ.grad.model.transcript.TranscriptResult result : transcript.getResults(code)) {
     ...
     */
    public static void adapt(
            final ca.bc.gov.educ.grad.model.achievement.Achievement achievement,
            final Student student) {
        ...
        for (final ca.bc.gov.educ.grad.model.achievement.AchievementResult result : achievement.getResults(code)) {
            // Transcipt results immeadiately added to student for report generation.
    ...
    private static TranscriptResult adapt(
            final ca.bc.gov.educ.grad.model.transcript.TranscriptResult tr) {
        validate(tr, "transcript result");

        final ca.bc.gov.educ.grad.model.transcript.Course c = tr.getCourse();
        final ca.bc.gov.educ.grad.model.transcript.Mark m = tr.getMark();
    ...
    private static AchievementResult adapt(
            final ca.bc.gov.educ.grad.model.achievement.AchievementResult tr) {
        validate(tr, "transcript result");
     ...
     */
    public static void adapt(
            final ca.bc.gov.educ.grad.model.exam.Exam exam,
            final Student student) {
        validate(exam, "exam");
        validate(student, "student");

        for (ca.bc.gov.educ.grad.model.exam.ExamResult result : exam.getResults()) {
    ...
    private static ExaminationResult adapt(
            final ca.bc.gov.educ.grad.model.exam.ExamResult er) {
        validate(er, "exam result");

        final ca.bc.gov.educ.grad.model.transcript.Mark m = er.getMark();
        validate(m, "mark");
    ...
    public static AcademicAward adapt(
            final ca.bc.gov.educ.grad.model.transcript.GraduationData academicAward,
