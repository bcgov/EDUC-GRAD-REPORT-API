...
public class RESTServicePOSTTest {
    ...
    @Before
    public void setUp(){
        Mockito.reset(reportApiClient, responseMock, requestHeadersMock, requestBodyMock, requestBodyUriMock);

        ThreadLocalStateUtil.clear();

        when(this.reportApiClient.post()).thenReturn(this.requestBodyUriMock);
    ...
    public void testPost_GivenProperData_Expect200Response(){
        ...
        byte[] response = this.restService.post(TEST_URL, TEST_BODY, byte[].class, reportApiClient);
    ...
    public void testPost_Given4xxErrorFromService_ExpectServiceError() {
        ...
        this.restService.post(TEST_URL, TEST_BODY, byte[].class, reportApiClient);
    }
