...
import ca.bc.gov.educ.grad.utils.*;
...
public class GradReportSignatureController extends BaseController {
    ...
    public ResponseEntity<GragReportSignatureImage> getSignatureImageByCode(@PathVariable String cpCode) {
        String _m = String.format("getSignatureImageByCode(String %s)", cpCode);
        logger.debug("<{}.{}", _m, CLASS_NAME);
    	logRequest();
        return response.GET(gradReportSignatureService.getSignatureImageByCode(cpCode));
    }
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "OK")})
    public ResponseEntity<ApiResponseModel<GragReportSignatureImage>> saveGragReportSignatureImage(@RequestBody GragReportSignatureImage signatureImage) {
        ...
        return response.UPDATED(gradReportSignatureService.saveSignatureImage(signatureImage));
    }
}
